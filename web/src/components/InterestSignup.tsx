...
export function InterestSignup({ compact = false }: { compact?: boolean }) {
    ...
    return (
        <div className={compact ? "px-2" : "text-center"}>
            {!compact && (
                <>
                    <h3 className="text-sm font-medium text-gray-900">{t("interest.title")}</h3>
            ...
            <form onSubmit={handleSubmit} className={compact ? "flex flex-col gap-2" : "flex gap-2"}>
                ...
                >
                    {status === "submitting" ? "..." : t("interest.notifyMe")}
                </button>
            </form>
            {status === "error" && (
                <p className="text-xs text-red-600 mt-2 text-center">{t("interest.error")}</p>
            )}
        </div>
    );
}
